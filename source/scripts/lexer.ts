            ...
            for (var i = 0; i < sourceByLines.length; i++) {
                ...
                if (!(matched === null)) {
                    ...
                    for (var j = 0; j < matched.length; j++) {
                        ...
                        // Not a keyword? Maybe an identifier or character
                            ...
                            for (var idIndex = 0; idIndex < currentToken.length; idIndex++) {
                                if (lexingString) {
                                    var token = Token.makeNewToken('CHARACTER', currentToken[idIndex], i + 1);
                                    _Logger.logMessage("Created '" + token.value + "' character token.");
                                } else {
                                    var token = Token.makeNewToken('IDENTIFIER', currentToken[idIndex], i + 1);
